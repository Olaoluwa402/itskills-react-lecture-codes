import React, { Component } from 'react'
 ...
 class FormValidate extends Component {
    ...
    validate = ()=>{
        ...
        if(this.state.touched.email && this.state.email.length > 0 && pattern.test(this.state.email) === false){
          console.log('fired')
    ...
    return (
      <div>
        <form onSubmit={this.submitHandler.bind(this)} noValidate>
            {errorFirstName && <p style={{color:'red', fontSize:'0.9rem'}}>error: {errorFirstName}</p>}
            <input 
            type="text" 
            name='firstName' 
        ...
        {errorEmail && <p style={{color:'red', fontSize:'0.9rem'}}>error: {errorEmail}</p>}      
          <input 
            type="email" 
            name='email'
